...
def build_segment_profit(
    ...
    out.note = (
        ...
        if saw_table
        # **단일 부문이라고 단정하지 않는다.** 주석을 못 찾은 것과 부문이
        # 하나인 것은 다르다 — 파마리서치는 매출을 4개 부문으로 공시하면서
        # 영업부문 주석은 내지 않는다. 단정하면 같은 리포트의 「부문 구성」과
        # 정면으로 모순된다(실측).
        else (
            "동사는 영업부문별 손익을 공시하지 않았다. 부문별 수익성은 이 공시로 "
            "확인할 수 없어 전사 기준으로만 볼 수 있다."
        )
    )
    return out
