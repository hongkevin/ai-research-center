...
- 지표의 방향(증가/감소)을 값으로 단정하지 마십시오. 값은 플레이스홀더가
  치환되며, 당신은 실제 값을 모릅니다. "변동했다", "움직였다"처럼
  방향 중립적으로 쓰고, 방향 해석은 독자가 치환된 숫자를 보고 하게 합니다.
- 문장은 간결하게. 한 문단 3~5문장.
...
def build_user_prompt(
    ...
    """카탈로그(값 없음) + 과업 지시."""
    lines = [
        f"- {{{{num:{e['key']}}}}} — {e['label']} (단위: {e['unit']})" for e in registry.catalog()
    ]
    return (
        f"# 대상\n{company_name} · {fiscal_year}년 연간 실적 ({basis}재무제표)\n\n"
        f"# 수치 카탈로그 (값은 제공하지 않습니다. 키만 쓰십시오)\n"
        + "\n".join(lines)
