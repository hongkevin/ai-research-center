...
| ~~Peer 비교표~~ | ~~다종목 처리 + 업종 분류가 별건~~ → **전제가 틀렸다. 만들었다** (`finmodel/peer.py`·`peer_suggest.py`). 「업종 분류가 별건」이 맞는데, **업종 분류를 안 하고 푸는 길**이 있었다 — 씨앗 종목의 상관으로 후보를 낸다. 실측 근거는 `peer_suggest.py` 독스트링 |
| 3표 완전 모델 (BS·CF 추정) | 손익 추정을 먼저 실측 검증한 뒤 |
| 카피라이팅 제목 | 품질 편차 + 컴플라이언스 위험 |
| Vercel 배포 · 프런트/백 분리 | 서버리스라 `.arc-store`가 사라져 revision 추적이 죽는다. 화면이 Next.js가 된 뒤에도 **정적 익스포트를 FastAPI가 서빙**해 컨테이너 하나를 유지한다 (D37) — LLM 예산이 같은 프로세스에 있다. corpCode 캐시는 D69에서 디스크로 내려 더는 근거가 아니다 |
